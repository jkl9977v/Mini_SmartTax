...
public class StartEndPageService {
	public StartEndPageDTO execute(int page, int limit, String searchWord, String userNum) {
		int startRow = ((page - 1) * limit) + 1 ; // 11
		int endRow = startRow + limit - 1; // 11 + 10 -1
		StartEndPageDTO sepDTO = new StartEndPageDTO(startRow, endRow, searchWord, userNum);
		return sepDTO;
	}
